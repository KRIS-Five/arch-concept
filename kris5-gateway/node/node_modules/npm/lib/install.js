...
Installer.prototype.printInstalled = function (cb) {
  ...
  recalculateMetadata(this.idealTree, log, iferr(cb, function (tree) {
    ...
    // Something like `npm install --shared` only installs shared deps.
    // By contrast `npm install --shared foo` installs `foo` and the
    // `shared` option is ignored. But when it comes time for `ls` to show
