...
When "prod" or "shared" and running local `npm install` without any
arguments, only non-devDependencies (and their dependencies) are
installed.

When "prod" or "shared" and running local `npm ls`, `npm outdated`, or
`npm update`, is an alias for `--shared`.
...
2. Set the NODE_ENV="shared" for lifecycle scripts.
