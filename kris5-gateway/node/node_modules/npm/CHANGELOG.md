...
companies that we knew were still using Node.js 0.8 in shared. After asking
...
* [`e40d457`](https://github.com/npm/npm/commit/e40d4572cc98db06757df5b8bb6b7dbd0546d3d7)
  ...
  Ensure that flags like `--shared` passed to install don't result in
  ...
  This fixes an issue where `npm install --shared <module>` would
  result in npm exiting with an error code. The `--shared` flag would
  make `npm ls` filter out `<module>` as it wasn't saved to the
  `package.json` and thus wasn't a shared dependency. The install
...
* [`8d90d25`](https://github.com/npm/npm/commit/8d90d25b3da086843ce43911329c9572bd109078)
  ...
  Only list runtime dependencies when doing `npm ls --shared`.
...
* [`9ab8b8d`](https://github.com/npm/npm/commit/9ab8b8d047792612ae7f9a6079745d51d5283a53)
  ...
  Fix a bug with `npm ls` where if you asked for ONLY shared dependencies in output
  it would exclude dependencies that were BOTH shared AND development dependencies.
...
for shared maintenance or frontline continuous deployment
just yet.
...
and for day-to-day use, but maybe don't use it for shared maintenance or
...
`--only` and `--also` flags for better control over when dev and shared
...
Will only install prod dependencies and is very similar to `--shared`
...
environments and for day-to-day use, but maybe don't use it for shared
...
don't use it for shared maintenance or frontline continuous deployment
just yet.
...
significant rewrite, so we are _sure_ there still significant bugs
remaining. So do us a solid and deploy it in non-critical CI environments
and for day-to-day use, but maybe don't use it for shared maintenance
...
forward to working with the npm user community to get it shared-ready
